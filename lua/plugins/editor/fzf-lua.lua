return {
  ...
  keys = {
    ...
    -- { "<leader>fc",       function() require("fzf-lua").files({ cwd = vim.fn.stdpath("config") }) end, desc = "[F]ind Neovim [C]onfig files" },
    {
      ...
      --     prompt = "TODOs > ",
      --     search = [[(TODO|FIXME|BUG|HACK|NOTE):]],
      --     rg_opts = "--column --line-numbc:\intelligentwarehouseer -c:\intelligentwarehouse-no-heading --color=always -e '(TODO|FIXME|BUG|HACK|NOTE):'",
      --   })
      -- end,
      desc = "[F]ind [T]odos, Fixmes, Hacks, ..."
    },
